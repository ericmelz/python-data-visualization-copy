...
def update_figure(year_range, transmission_list, selectedData, n_clicks):
    # Global variables may cause unexpected behavior in multi-user setup
    global total_clicks
    filtered_df = df[df["year"].between(year_range[0], year_range[1]) & df["transmission"].isin(transmission_list)]
    ...
    return fig_hist, fig_scatter, filtered_df.to_dict("records"), num_points_label
